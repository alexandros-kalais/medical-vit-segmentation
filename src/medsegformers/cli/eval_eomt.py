...
class EoMTSemanticWrapper(nn.Module):
    ...
    def __init__(self, eomt: EoMT, img_size: tuple[int, int], num_classes: int):
        super().__init__()
        self.eomt = eomt
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        mask_logits_list, class_logits_list = self.eomt(x)  
...
def main():
    ...
    for exp_id in exp_ids:
        ...
        if num_classes == 2:
            eomt_num_classes = num_classes - 1
        else:
            eomt_num_classes = num_classes
        ...
        model = EoMTSemanticWrapper(network, img_size=(H, W), num_classes = num_classes).to(device).eval()
        ...
        print(f"[OK] {exp_dir.name}  mIoU={out['mean_miou']:.4f}  mHD95={out['mean_hd95']:.4f}  "
            f"GFLOPs={out['gflops']:.2f}  FPS={out['fps']:.1f}  params={out['total_params']}")
