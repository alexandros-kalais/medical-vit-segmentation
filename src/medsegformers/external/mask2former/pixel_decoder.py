...
from medsegformers.external.utils.batch_norm import get_norm
from medsegformers.external.utils.position_encoding import PositionEmbeddingSine
from medsegformers.external.utils.transformer import _get_clones, _get_activation_fn
from medsegformers.external.utils.ms_deform_attn import MSDeformAttn
...
class MSDeformAttnPixelDecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if self.transformer_num_feature_levels > 1:
            ...
            for in_channels in transformer_in_channels[::-1]:  # TODO remove [:-1]
    ...
    def forward_features(self, features):
        ...
        for idx, f in enumerate(self.transformer_in_features[::-1]):  # TODO remove [:-1]
        ...
        # Reverse feature maps into top-down order (from low to high resolution)
        for idx, f in enumerate(self.in_features[:self.num_fpn_levels][::-1]):  # TODO re put [:self.num_fpn_levels][::-1]):
            x = features[f].float()
