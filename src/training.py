import argparse
from datasets.hyperkvasirDataset import HyperKvasirDataset
from datasets.EndoscopyDataset import EndoscopyDataset
...
def get_args_parser():
    ...
    parser.add_argument("--experiment-id", type=str, default="unet-test-run", help="Experiment ID for Weights & Biases")
...
def create_dataset(args):
    ...
    if args.dataset == "hyperkvasir":
        dataset_root = Path(args.data_dir) / "HyperKvasir"
        num_classes = 1
        ...
        dataset_root = Path(args.data_dir) / "endoscapes_segmentation_dataset\endoscapes_segmentations_processed"
        ratio = (0.7, 0.2, 0.1)
...
def train(args):
    ...
    output_dir = os.path.join("checkpoints", args.dataset, args.experiment_id)
